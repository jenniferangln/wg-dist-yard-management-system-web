...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
	...
	try {
		const response = await http.put<Response<MYardActivity, string>>(`yard-activities/${id}`, data)
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
	const id = (await params).id
	try {
		const response = await http.delete<Response<MYardActivity, string>>(`yard-activities/${id}`)
		return NextResponse.json({ message: response.data.message })
	} catch (error) {
		const err = error as AxiosError<Response<null | { listMessage: string[] }, string>>
		return NextResponse.json(
